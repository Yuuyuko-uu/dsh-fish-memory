...
function evaluate(m, query, now, policy, 尺) {
  ...
  const mText = 词面(m)
  // ⚠⚠⚠ 2026-09-22 我在这里踩了一个坑，记下来免得再犯：
  //   我一度把匹配改成「按词集」（`m词集.has(k)`），想治「单字撞进长词」。
  //   结果**撞坏了正经召回**：`中文片段('她的胃不好…')` 会**丢掉开头的单字**
  //   （「她的」被切掉），于是问句「她的胃」只剩一个单字「胃」，
  //   而记忆那边只有「胃不」—— 按词表**根本对不上**，C 直接变 0 ✗
  //   原来那句 `includes` **正好补上了切词器的这个信息损失**。
  //   → 匹配保持 `includes` 不动；假词的问题改用**别的办法**治（见下面说话稀有度）✓
  ...
  for (const k of qKeys) {
    if (mText.includes(k)) s += 稀有度(尺.df, k) * 说话稀有(k)
  }
...
// ⚠⚠ 2026-09-22（反馈：「一二点是重复内容」）：**召回用的门槛要另定，不能用 0.85** ✓
//
//   实测她那两对「明明是一件事、却一起注进来」的：
//     胃寒那对      像度 0.833   ← 差 0.017 没够着 0.85 ✗
//     09-17 那对    像度 0.846   ← 差 0.004 ✗
//   为什么总差一点点：像度是**三字片段**，两句话只要换几个字，
//   整串片段就**全错位**（「她不是」「不是胃」「是胃寒」……全是跨词边界的碎片）✗
//
//   为什么不干脆把 0.85 调低：**0.85 是给「查重合并」定的** ——
//   那个是真删东西、不可逆，必须保守（她还被误合过一次）。
//   而召回时的去重**只是这一轮不显示它**，东西还在库里、下一轮照样能想起来 ——
//   风险小得多，就该用更宽的门槛 ✓
//   → 两个门槛分开：合并 0.85，召回 0.80
const 召回重复门槛 = 0.8
