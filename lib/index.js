    ...
    const 找重复组 = (memories) => {
      ...
      return 组们
        .map((g) => {
          ...
          // ⭐ 2026-09-23：**「真重复」要两条都满足**（原来只看长度）——
          //   ① 长度差不多（一长一短是「汇总+明细」，合了会丢信息）
          //   ② 而且**词像度也够高**（≥真重复门槛）
          //   候选门槛降到 0.75 之后，光看长度会把 0.76 的也放进「一键合」——
          //   那太松了，自动合的那一档必须守住 ✓
          const 最不像 = Math.min(...g.slice(1).map((x) => x.像))
          return {
            类型: Math.min(...长们) / Math.max(...长们) >= 0.8 && 最不像 >= 真重复门槛 ? '真重复' : '汇总明细',
            条们: g.map((x) => ({
              id: x.m.id,
