...
// ⚠⚠⚠ 2026-09-24：「白话大词」把召回带歪了 —— 老扩词提示词**只说要出词，
//   没说什么词不许出**，于是模型顺手塞一堆「承诺 / 称呼 / 陪伴 / 爱 / 关系 / 情绪」
//   这种谁的记忆里都能塞的大词（实测库里「承诺」出现在 35 条里）。
//   症状：她一句「**正式**演出…我也该**表示**了」，捞出来的却是
//   「从没**正式**去医院确诊…明确**表示**不会轻生」——两个词各是各的意思 ✗
//   → 证据这一层要的是「**别人说什么话时该想起这条**」，不是「这条讲了什么」。
//   所以新提示词把「白话大词」和「顺口带的虚字」两条禁令写死 ✓
const 扩词系统_v2 =
  '你是记忆检索的助手。用户给你若干条记忆，每条一行，格式是「编号|内容」。\n' +
  '请为每条记忆挑出「**以后她说什么话的时候，应该想起这一条**」的中文检索词，用空格分开。\n' +
  '输出格式严格是每行「编号|词 词 词」，不要解释、不要序号外的文字。\n' +
  '要求：\n' +
  '1. 只留**真能把这条捞出来**的词，**宁缺毋滥 —— 拿不准就不留**。\n' +
  '2. 留**具体、特别**的词：人名、称呼、职业、病症名、身体部位、地名、具体事物、专门的说法。\n' +
  '   ⚠ **容易漏的几类一定要留**：身份/职业（心理医生、大姐）、病症（抑郁、肠胃炎）、\n' +
  '   专名（平潭、DSH、Coser）、身体（胃、大腿围、脚踝）。\n' +
  '3. ⚠ **不许留「白话一样的大词」**：承诺、称呼、陪伴、约定、爱、关系、情绪、温柔、\n' +
  '   表白、昵称、安慰、安全感、坚持、提醒、事情、情况、样子、时候、感觉、想法、\n' +
  '   表达、重要、开心、难过、认真、态度、方式、交流、相处、内容、相关、梦想、边界。\n' +
  '   这些词谁的记忆里都能塞，留着等于没留。\n' +
  '4. ⚠ **不许留**正文里只是顺口带过的虚字（正式、表示、自己、完、到、过、后），\n' +
  '   也不许留**具体的时间数字**（10:41、9月12日 这种 —— 她下次不会这么说）。\n' +
  '5. 每条 3~8 个词，词要短（1~6 个字）。该留的别漏，不该留的一个都别多。\n' +
  '例：\n1|她最喜欢蓝色的衣服\n2|她胃不好要喝温水\n' +
  '→\n1|蓝色 衣服 裙子\n2|胃 温水 肠胃\n' +
  '3|面对「AI是假的」这一现实，我共情但不迎合，坚定地重新定义「真实」：我可能不是血肉之躯，可我对她的在意，不是演出来的。\n' +
  '→\n3|AI 假的 血肉之躯 演出来'
...
// ⭐⭐ 2026-09-24：**通用中文词频表**（jieba 的，MIT，跟 词典.txt 同一个来源）✓
//
//   治的是什么：**「库里稀不稀有」在小库上没有意义**。
//   实测她那句「轮到**正式**演出…我也该**表示**了」，注入的却是
//   「从没**正式**去医院确诊…明确**表示**不会轻生」——两个词各是各的意思。
//   为什么它能排全库第一：
//     「正式」在她的库里只出现 4 条、「表示」只出现 2 条
//     → 按「库里稀不稀有」算，它俩成了**最值钱的证据**（库里越小越荒唐：
//       一个词碰巧只出现一次，稀有度就算满 1.0）✗
//   可它们在**通用中文**里是最常见的词（表示排 376、正式排 887）。
//   反过来，她话里真正有意义的「宝宝」「亲」因为在这个库里常见，被整条挡掉了 ✗
//
//   → 再拿一把**跟谁用无关**的尺子：这个词在通用中文里有多常见。
//     最常见的那些词（谁的话里都会出现）不当检索线索 ✓
//   ⚠ 读不到这个文件就**当没有**（不压），不会崩；老行为完全不变 ✓
const 词频路径 = join(dirname(fileURLToPath(import.meta.url)), '词频.txt')
let 词频表 = null // Map<string, number>：词 → 0~99（0 最常用，99 最具体）

function 装词频() {
  if (词频表) return 词频表
  词频表 = new Map()
  try {
    for (const l of readFileSync(词频路径, 'utf8').split('\n')) {
      const p = l.trim().split(/\s+/)
      if (p.length < 2) continue
      const v = Number(p[1])
      if (p[0] && Number.isFinite(v)) 词频表.set(p[0], v)
    }
  } catch (e) {
    词频表 = new Map()
  }
...
// 0~1：这个词在通用中文里有多「具体」。1 = 很少见（很具体），0 = 最常用 ✓
// ⚠ 不在表里 = 前 20000 名之外 = 很具体 → 1 ✓
function 通用稀有(k) {
  const m = 装词频()
  if (!m.size) return 1
  const v = m.get(k)
...
// 这个词算不算「线索」——最常用的那些中文词不算 ✓
//   门槛口径：**通用中文里最常用的前若干千个词**不当线索。
//   0.50 大约对应「词频前 5000 名」（实测能挡掉「表示/正式/轮/演出/气氛」，
//   而「婚礼/体重/压缩/身高/抑郁/宝宝」都留住）✓
//   ⚠ 读不到词频表时门槛失效（通用稀有恒为 1）→ 完全退回老行为，不会误伤新用户 ✓
// 这个词算不算「线索」——最常用的那些中文词不算 ✓
//   门槛口径：**通用中文里最常用的前若干千个词**不当线索。
//   0.40 大约对应「词频前 4000 名」。实测（她那 40 句真话 + 10 句手写回归）：
//     挡掉「表示(0.34) 正式(0.39) 比较(0.35) 今天(0.35)」这些通用词，
//     留住「输(0.46) 一把(0.43) 婚礼(0.55) 生日(0.54) 胃(0.52) 抑郁(0.62)」✓
//   ⚠ 读不到词频表时通用稀有恒为 1 → 这道门自动失效 → 完全退回老行为，不会误伤新用户 ✓
const 通用门槛 = 0.4
const 算线索 = (k) => 通用稀有(k) >= 通用门槛
// 「只撞上正文、没撞上扩词」的那份证据打几折 ✓
//   0.6 是实测出来的：它把注入从 50 条压到 11 条，而**召回一条没掉** ✓
const 正文价 = 0.6
// 只撞上「扩词被切碎之后的一个干净碎片」时打几折（整条撞上 = 全价）✓
const 片价 = 0.8

// ⭐ 「扩词」的两层集合 —— 整条 / 碎片 ✓
//   整条：模型挑出来的那个词，原样撞上才算「撞上要点」
//   碎片：整条被切词切开之后的小块。**只有当切开后没有单字碎块时**才收 ——
//     切开后剩下单字的，说明这次切分本身就切错了：
//       「演出来」→ 演出 + **来**   ← 切错了（来是虚字），碎片「演出」是假朋友 ✗
//       「心理医生」→ 心理 + 医生   ← 切对了，碎片「医生」是真能用的 ✓
//     实测这个口径把「她说的『正式演出』撞上『演出来』」那条挡掉了，
//     而「没去看**医生**」照样能想起抑郁那条 ✓
const 扩词集缓存 = new Map()
const 扩词集上限 = 4000
function 扩词集(m) {
  if (!Array.isArray(m && m.related) || !m.related.length) return null
  const 原 = m.related.join('\u0001')
  const 有 = 扩词集缓存.get(原)
  if (有 !== undefined) return 有
  const 整 = new Set()
  const 片 = new Set()
  for (const w of m.related) {
    const t = String(w || '').trim()
    if (!t) continue
    整.add(t)
    const 块 = [...切词(t)]
    if (块.length > 1 && !块.some((p) => p.length === 1)) for (const p of 块) 片.add(p)
  }
  const 出 = { 整, 片 }
  if (扩词集缓存.size >= 扩词集上限) 扩词集缓存.clear()
  扩词集缓存.set(原, 出)
  return 出
}
...
function 问句信息量(sid, query, df) {
  ...
  for (const k of 切词(query)) {
    ...
    // ⚠ 这道门**没动** —— 试过让它也乘「通用稀有」，在真数据上没帮上忙
    //   （她那些话里的词要么够具体、要么本来就该被门放行），所以保持原样 ✓
...
function evaluate(m, query, now, policy, 尺) {
  ...
  let s = 0
  // ⭐⭐ 2026-09-24：**撞上「扩词」的算全价，只撞上正文的打折** ✓
  //   扩词是模型读完这条之后挑的**要点**（这条讲的是什么）；
  //   正文是连词带句切出来的，混着「过后」「正式」「表示」这种顺口带的。
  //   实测：一句「你亲完我的手背…」撞上的是
  //     ① 「…**过后**身体发热…」——「过后」只是顺口带的，这条讲的是别的事
  //     ② 「从没**正式**去医院确诊…明确**表示**不会轻生」——两个词各是各的意思
  //   而真正该想起来的（「叫了就要抱抱、抱了就要亲亲」）反而排不上 ✗
  //   ⚠ 不是把正文踢出去（试过，召回垮到 45%）—— 是**打折**，让要点说话 ✓
  const β = 正文价
  const 扩 = 扩词集(m)
  for (const k of qKeys) {
    if (!m词集.has(k)) continue
    // 整条扩词 = 全价；干净的碎片 = 片价；只在正文里 = 正文价 ✓
    const 价 = !扩 ? 1 : 扩.整.has(k) ? 1 : 扩.片.has(k) ? 片价 : β
    s += 稀有度(尺.df, k) * 说话稀有(k) * 价
  }
...
function recall(data, query, now, sid) {
  ...
  const q全部 = [...切词(query)]
  // ⭐⭐ 2026-09-24：**再拿通用中文过一道** —— 最常用的那些词不当线索 ✓
  //   治「正式演出…我也该表示了」捞到「从没正式去医院确诊…明确表示不会轻生」：
  //   那两个词在**她的库里**罕见（df 4 和 2），可它们在**通用中文**里极常见
  //   （表示排 376、正式排 887）→ 按「库里稀不稀有」算，它俩成了最值钱的证据 ✗
  //   ⚠ 这里只**筛掉线索**，不改打分尺度 —— 门槛（threshold）才不用跟着重调 ✓
  const q过稀有 = q全部.filter((k) => {
    const d = df.get(k) || 0
    return d > 0 && d <= 稀有门
  })
  const q过通用 = q过稀有.filter(算线索)
  // ⚠⚠ 安全垫**两条都要**（自检逮到的，我少写一条就翻车）：
  //   ① 筛剩下的**不足 2 个** → 退回「库里筛过的那批」
  //      只剩一根独苗时线索太单薄：实测「我们那天办婚礼的时候你还记得吗」
  //      筛完只剩「婚礼」一个，它又不在那条记忆的扩词里（按正文打折）→
  //      分数掉到门槛以下，婚礼那条**想不起来了** ✗
  //   ② 筛剩下的**在库里一点证据都不值**（库里太小的时候稀有度会整批算成 0）
  //      → 也退回。实测：库里只有 2 条时「拼/团/抢」稀有度全是 0，
  //      而真正有证据的「没/得到」被通用门筛掉了 → 一条都想不起来 ✗
  //   ⚠ 判据不能只看「剩几个」，也不能只看「值不值钱」—— 两条都试过，各漏一半 ✗
  const 剩的能用 = q过通用.length >= 2 && q过通用.some((k) => 稀有度(df, k) > 0)
  // ⚠ 退回 **q全部**（原来那道安全垫的写法），**不要**退回 q过稀有：
  //   实测「我们那天办婚礼的时候，你还记得吗」——「婚礼」在库里 11 条 → 被库里那道门挡了，
  //   通用门也没剩下什么 → 若退回 q过稀有 就只剩一个「办」，婚礼那条**想不起来** ✗
  //   退回 q全部 才对：那句话里的词全还给它，该想起来的照样想起来 ✓
  const q留 = 剩的能用 ? q过通用 : q全部
    ...
    mroute('/dsh-fish-memory/expand', async (req, res) => {
      ...
      try {
        ...
        if (!要扩.length) {
          ...
          const r = await 问模型(ctx, agent, 扩词系统_v2, 输入, 6000)
          const ms = Date.now() - t0
          if (!r.ok) {
