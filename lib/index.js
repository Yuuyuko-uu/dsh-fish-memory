...
const VERSION = '0.9.0' // 改一次 +0.0.1，末位满 10 进位 ✓
      ...
      try {
        ...
        if (!要判.length) {
          ...
          const 系统 =
            ...
            '**重要的日子（生日、纪念日、结婚、第一次）**、长期的经济状况、家里的情况。\n' +
            '0.5 = 平常的：发生过的事、当时的想法、一时的心情。\n' +
            // ⚠ 这条最要紧 —— 反馈就是「某天流水账被判成了永远不淡」
            '⚠ **记着「某一天具体发生了什么」的流水账（几点睡、吃了什么、那天做了什么），一律 0.5**，\n' +
            '  哪怕它写在「关系时间线」这种听起来重要的标题底下。\n' +
            '  只有**长期成立**的才给 1：比如「我们约定每天十点半上床」是 1，「09-19 那天十点半上床」是 0.5。\n' +
            '拿不准就回 0.5。\n' +
