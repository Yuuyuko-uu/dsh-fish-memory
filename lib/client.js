...
const 洗键进度 = { 已: 0, 总: 0, 在跑: false, 干跑: false }
    ...
    function Section() {
      ...
      if (open) {
          ...
          React.createElement(
            ...
            React.createElement(
              ...
              m.已被取代
                ...
                : null,
              // ⭐⭐ 2026-09-23：**事实键** —— 行里挂个小牌子，
              //   同一个键的条目一眼能认出来（排在一起看的时候有用）✓
              m.事实键
                ? React.createElement(
                    'div',
                    {
                      style: { flex: 'none', fontSize: 11, marginRight: 10, padding: '1px 6px', borderRadius: 8, background: 'rgba(120,170,220,.16)', color: '#7aaadc', whiteSpace: 'nowrap' },
        ...
        const 没键的 = (s.memories || []).filter((m) => !m.已被取代 && !m.refinedSkip && !m.事实键).length
          ...
          const 跑 = () => {
            ...
            post('/wash-keys', 体)
              .then((r) => {
                ...
                setNotes([
                  '正在洗… 已过 ' + 累计洗 + ' 条：补上键 ' + 累计给 + ' 条、留空 ' + 累计空 + ' 条' +
                    (累计废 ? '、顺手作废 ' + 累计废 + ' 条' : '') +
                    (r.done ? '' : '，还剩 ' + r.剩 + ' 条'),
                ])
                if (!r.done) {
                  if (连着空 >= 3) {
                    ...
                    setBusy(false)
                    setNotes([
                      '⚠ 连着 3 批模型都没回出键，先停了（还剩 ' + r.剩 + ' 条）。',
                      '已经补好的**都存下来了** —— 再点一次会接着跑 ✓',
                    ])
                    return
                  }
                  setTimeout(跑, 1500)
                  ...
                  setBusy(false)
                  setNotes([
                    '✓ 洗完了：过了 ' + 累计洗 + ' 条 —— 补上键 ' + 累计给 + ' 条、留空 ' + 累计空 + ' 条' +
                      (累计废 ? '，其中 ' + 累计废 + ' 条因为「同一个键只留最新」被标成「已被取代」（还留得住、能恢复）' : ''),
                    '留空的那批保持原样，一条都没动 ✓（要退回来就点「把补的键全摘掉」）',
                  ])
                  load()
                }
              })
