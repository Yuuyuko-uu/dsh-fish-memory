    ...
    function Section() {
      ...
      if (open) {
          ...
          const 跑 = () => {
            ...
            post('/backfill-stable', 体)
              .then((r) => {
                ...
                if (!r.done && (r.判了 || 0) > 0) setTimeout(跑, 1500)
                else {
                  补标进度.在跑 = false
                  setBusy(false)
                  setNotes([
        ...
        const 扩词区 = () => {
          ...
          // ⭐ 2026-09-22（反馈「没有弹进度条」）：补标不淡也配一条 ✓
          if (补标进度.在跑 && 补标进度.总 > 0) {
            const 百 = Math.round((补标进度.已 / 补标进度.总) * 100)
            小件.push(
              React.createElement(
                'div',
                { key: 'bbar', style: { margin: '8px 0' } },
                React.createElement(
                  'div',
                  { style: S.barOut },
              ...
              ? React.createElement(
                  ...
                  showDeleted && 已删.length > 0 && (!confirm || confirm.kind !== 'purgeall')
                    ? React.createElement(
                        ...
                        // ⭐ 2026-09-22（反馈「没有一个一键全选呀，回收站」）：**全选 / 取消全选** ✓
                        React.createElement(
                          'button',
                          {
                            style: S.btn,
