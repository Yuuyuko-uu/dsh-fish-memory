    ...
    function Section() {
      ...
      if (open) {
        ...
        const 补不淡 = () => {
          ...
          let 累计降 = 0
          const 跑 = () => {
            post('/backfill-stable', { session: s.id, 每批: 20 })
              .then((r) => {
                if (!r || !r.ok) {
                  setBusy(false)
                  setNotes(['补标失败：' + ((r && r.message) || '？')])
                  return
                }
                累计判 += r.判了 || 0
                累计升 += r.改成不淡 || 0
                ...
                if (!r.done && (r.判了 || 0) > 0) setTimeout(跑, 300)
                else {
                  setBusy(false)
                  setNotes([
                    '✓ 判完了：看了 ' + 累计判 + ' 条 —— ' + 累计升 + ' 条标成「永远不淡」（收在「▸ 长期事实」那栏），' +
                      累计降 + ' 条从「永远不淡」改成会淡',
            ...
            ? React.createElement(
                ...
                confirm && confirm.kind === 'stable'
                  ? React.createElement(
                      ...
                      React.createElement('button', { style: S.btnDanger, disabled: busy, onClick: 补不淡 }, busy ? '判着呢…' : '开始'),
                      React.createElement('button', { style: S.btn, onClick: () => setConfirm(null) }, '算了'),
                    )
                  : null,
          ...
          React.createElement(
            ...
            showDeleted
              ? React.createElement(
                  ...
                  showDeleted && 已删.length > 0 && (!confirm || confirm.kind !== 'purgeall')
                    ? React.createElement(
                        'div',
                        { style: { marginTop: 10 } },
