...
const 洗键进度 = { 已: 0, 总: 0, 在跑: false, 干跑: false }
    ...
    function Section() {
      ...
      if (open) {
        ...
        const 一条记忆 = (m) =>
          React.createElement(
            ...
            m.related && m.related.length
              ...
              : null,
            // ⭐⭐ 2026-09-23：**事实键** —— 挂在这儿（第二行小字），不占行里的宽度 ✓
            //   同一个键的条目一眼能认出来；点一下能把键抄下来（想搜同键的条目时有用）
            m.事实键
              ? React.createElement(
                  'div',
                  ...
                  React.createElement(
                    'span',
                    {
                      ...
                      title: '事实键：' + m.事实键 + ' —— 同一个键的只留最新那条，旧的会标成「已被取代」',
                    },
                    m.事实键,
                  ),
                )
        ...
        const 有键的 = (s.memories || []).filter((m) => m.事实键).length
          ...
          const 跑 = () => {
            ...
            post('/wash-keys', 体)
              .then((r) => {
                ...
                if (干跑) {
                  // 干跑：把几个例子摆出来，让她看一眼键给得对不对 ✓
                  洗键进度.在跑 = false
                  setBusy(false)
                  const 例 = (r.示例 || []).map((x) => '　' + x.键 + '　←　' + x.正文)
                  setNotes(
                    [
                      '先试了 ' + (r.洗了 || 0) + ' 条（**一个字都没改**）：给了键 ' + (r.给了键 || 0) + ' 条、留空 ' + (r.留空 || 0) + ' 条',
                      '它给的键长这样：',
                    ]
                      .concat(例.length ? 例 : ['　（这批全留空了）'])
                      .concat(['', '看着行就点「真的开始洗」；不行就先别动 —— 这一步不做也不影响别的 ✓']),
                  )
                  return
                }
                setNotes([
                  '正在洗… 已过 ' + 累计洗 + ' 条：补上键 ' + 累计给 + ' 条、判留空 ' + 累计空 + ' 条' +
                    (累计废 ? '、顺手作废 ' + 累计废 + ' 条' : '') +
                    (r.done ? '' : '，还剩 ' + r.剩 + ' 条'),
                ])
                if (!r.done) {
                  if (连着空 >= 3) {
                    ...
                    setBusy(false)
                    setNotes([
                      '⚠ 连着 3 批模型**整批都没回出东西**，先停了（还剩 ' + r.剩 + ' 条）。',
                      '已经洗好的**都存下来了** —— 再点一次会接着跑 ✓',
                      '（要是它只是把这批判成了「留空」，那不算出错、会自己往下走）',
                    ])
                    return
                  }
                  setTimeout(跑, 1500)
                  ...
                  setBusy(false)
                  setNotes([
                    '✓ 洗完了：过了 ' + 累计洗 + ' 条 —— 补上键 ' + 累计给 + ' 条、判了「留空」' + 累计空 + ' 条' +
                      (累计废 ? '，其中 ' + 累计废 + ' 条因为「同一个键只留最新」被标成「已被取代」（还留得住、能恢复）' : ''),
                    '判留空的那批**保持原样**，一个字都没动 ✓（要退回来就点「把补的键全摘掉」）',
                  ])
                  load()
                }
              })
              .catch((e) => {
                洗键进度.在跑 = false
                setBusy(false)
                setNotes(['清洗失败：' + ((e && e.message) || e), '已经补好的都存下来了 —— 再点一次会接着跑 ✓'])
              })
          }
          跑()
        }

        // 清洗补键：**整份撤回**（把自动补的键全摘掉、顺手标的作废全恢复）✓
        const 撤销洗键 = () => {
          setConfirm(null)
          setBusy(true)
          post('/wash-keys/undo', { session: s.id })
            .then((r) => {
              ...
              setNotes([
                '✓ 撤回完了：摘掉 ' + (r.摘了 || 0) + ' 个自动补的键、恢复 ' + (r.恢复 || 0) + ' 条被作废的',
                '（你**自己手工填的**键没动 —— 这个按钮只管自动补的那些）',
              ])
              load()
            })
            .catch((e) => {
        ...
        return React.createElement(
          ...
          看记忆
            ? React.createElement(
                  ...
                  没键的 > 0
                    ? React.createElement(
                        'button',
                        {
                          style: S.btn,
                          title:
                            '给还没「事实键」的老记忆补上键 —— 有了键，同一个键的只留最新那条，' +
                            '不用再靠「句子像不像」去猜。先「试几条」看一眼它给的键，再决定要不要真洗。' +
                            '会**调用模型**（走你 DSH 里那个），一批 8 条慢慢跑；动之前自动备份',
                  ...
                  有键的 > 0
                    ? React.createElement(
                        'button',
                        {
                          style: S.btn,
                          title:
                            '把**自动补的**键全摘掉、顺手标的「已被取代」全恢复 —— 回到洗之前。' +
                            '你自己手工填的键不会被这个按钮动',
                ...
                // ⭐⭐ 2026-09-23：洗过之后跟她交代一句「剩下的去哪儿了」✓
                //   模型会把一批判成「这条不用给键」（一次性的、拿不准的）——
                //   那种**不写键但算问过了**。不说明的话她会以为漏了/白跑了 ✗
                判留空的 > 0
                  ? React.createElement(
                      'div',
                      { style: Object.assign({}, S.dim, { fontSize: 11, margin: '4px 0 0' }) },
                      '（另外 ' + 判留空的 + ' 条，模型判了「不用给键」—— 一次性的、或者拿不准的。' +
                        '那种不改东西、也不再重复问 ✓）',
                    )
                  : null,
                ...
                confirm && confirm.kind === 'washkeys'
                  ? React.createElement(
                      ...
                      React.createElement(
                        'div',
                        { style: { flex: 1 } },
                        '给这 ' + 没键的 + ' 条还没「事实键」的老记忆补上键？',
                        React.createElement(
                          'div',
                          { style: Object.assign({}, S.dim, { fontSize: 12, marginTop: 2 }) },
                          '有了键，**同一个键的只留最新那条**，就不用再靠「句子像不像」去猜了。' +
                            '会**调用模型**（走你 DSH 里那个），一批 8 条慢慢跑，**只在设置页跑、不碰对话**。' +
                            '动之前自动备份。',
                        ),
                      ),
                      React.createElement('button', { style: S.btn, disabled: busy, onClick: () => 洗键(true) }, busy ? '跑着呢…' : '先试 8 条看看'),
                      React.createElement('button', { style: S.btnDanger, disabled: busy, onClick: () => 洗键(false) }, busy ? '跑着呢…' : '直接开始洗'),
                      React.createElement('button', { style: S.btn, onClick: () => setConfirm(null) }, '算了'),
                    )
                  : null,
                // 撤退：把自动补的键全摘掉
                confirm && confirm.kind === 'unwashkeys'
                  ? React.createElement(
                      ...
                      React.createElement(
                        'div',
                        { style: { flex: 1 } },
                        '把自动补的 ' + 有键的 + ' 个键全摘掉、被作废的也全恢复？',
                        React.createElement(
                          'div',
                          { style: Object.assign({}, S.dim, { fontSize: 12, marginTop: 2 }) },
                          '回到清洗之前的样子。**只摘自动补的** —— 你自己手工填的键不动。' +
                            '记忆条目本身一条都不会被删。',
                        ),
                      ),
                      React.createElement('button', { style: S.btnDanger, disabled: busy, onClick: 撤销洗键 }, busy ? '撤着呢…' : '摘掉'),
                      React.createElement('button', { style: S.btn, onClick: () => setConfirm(null) }, '算了'),
                    )
                  : null,
                // ⭐⭐ 2026-09-23：**清洗补键的进度条**（跟补标那条分开，不互相盖）✓
                洗键进度.在跑 && 洗键进度.总 > 0
                  ? (() => {
